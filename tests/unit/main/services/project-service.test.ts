...
describe('projectService', () => {
  ...
  describe('create', () => {
    it('should create a project with valid input', async () => {
      const project = await projectService.create({ name: '有效项目' })
      expect(project.name).toBe('有效项目')
    })

    it('should trim project name', async () => {
      const project = await projectService.create({ name: '  前后空格  ' })
      expect(project.name).toBe('前后空格')
    })

    it('should throw ValidationError for empty name', async () => {
      await expect(projectService.create({ name: '' })).rejects.toThrow(ValidationError)
    })
  ...
  describe('get', () => {
    it('should throw ValidationError for empty id', async () => {
      await expect(projectService.get('')).rejects.toThrow(ValidationError)
    })

    it('should throw NotFoundError for non-existent id', async () => {
      await expect(projectService.get('no-such-id')).rejects.toThrow(NotFoundError)
    })

    it('should return project by id', async () => {
      const created = await projectService.create({ name: '查找测试' })
  ...
  describe('list', () => {
    it('should return all projects', async () => {
      await projectService.create({ name: 'A' })
      await projectService.create({ name: 'B' })
  ...
  describe('update', () => {
    it('should throw ValidationError for empty id', async () => {
      await expect(projectService.update('', { name: 'x' })).rejects.toThrow(ValidationError)
    })

    it('should throw ValidationError for empty name in update', async () => {
      const created = await projectService.create({ name: 'original' })
      await expect(projectService.update(created.id, { name: '  ' })).rejects.toThrow(
        ValidationError
      )
    })

    it('should update successfully', async () => {
      const created = await projectService.create({ name: 'before' })
      const updated = await projectService.update(created.id, { name: 'after' })
  ...
  describe('delete', () => {
    it('should throw ValidationError for empty id', async () => {
      await expect(projectService.delete('')).rejects.toThrow(ValidationError)
    })

    it('should delete project', async () => {
      const created = await projectService.create({ name: 'to-delete' })
      await projectService.delete(created.id)
      await expect(projectService.get(created.id)).rejects.toThrow(NotFoundError)
    })
  })

  describe('archive', () => {
    it('should throw ValidationError for empty id', async () => {
      await expect(projectService.archive('')).rejects.toThrow(ValidationError)
    })

    it('should archive project', async () => {
      const created = await projectService.create({ name: 'to-archive' })
      const archived = await projectService.archive(created.id)
