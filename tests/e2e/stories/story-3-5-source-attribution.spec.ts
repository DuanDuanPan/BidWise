...
function advanceToProposalWriting(
  userDataPath: string,
  projectId: string
): void {
...
function buildMetadata(
  ...
  const attributions = PARA_TEXTS.map((text, i) => ({
    id: `sa-2-0-${i}`,
  ...
  const baselineValidations = opts?.skipBaseline
    ? []
    : [
        {
          id: 'bv-2-0-0',
...
test.describe('Story 3.5: Source Attribution & Baseline Validation', () => {
  ...
  test('@story-3-5 @p0 no-source paragraphs have yellow highlight', async () => {
    ...
    const paragraph = noSourceLabel.locator('xpath=ancestor::div[contains(@style, "FFFBE6")]').first()
