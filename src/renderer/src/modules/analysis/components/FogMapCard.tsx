...
export function FogMapCard({
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-gray-400">{expanded ? <DownOutlined /> : <RightOutlined />}</span>
      ...
      {/* Expanded detail */}
      {expanded && (
        <div className="border-t border-gray-100 px-4 pt-3 pb-4" data-testid="fog-map-card-detail">
          ...
          {item.suggestion && item.suggestion !== '无需补充确认' && (
            <blockquote className="mb-3 border-l-4 border-blue-300 bg-blue-50 px-3 py-2 text-sm text-gray-600 italic">
              {item.suggestion}
            </blockquote>
          )}
