...
export function MermaidElement(props: PlateElementProps): React.JSX.Element {
  ...
  const isNewNode = !node.source || node.source === DEFAULT_TEMPLATE
  ...
  const handleRenderError = useCallback((_error: string) => {
    // Error display is handled by MermaidRenderer
  }, [])
        ...
        {mode === 'editing' ? (
          <div className="bg-[#F0F5FF]" data-testid="mermaid-editing">
            ...
            <div className="border-b border-blue-200 p-3">
              <textarea
                className="w-full resize-y rounded border border-gray-300 bg-white p-3 font-mono text-sm leading-relaxed focus:border-blue-400 focus:outline-none"
                rows={8}
                value={localSource}
