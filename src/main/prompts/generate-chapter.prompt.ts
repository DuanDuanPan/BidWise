...
2. 占位符必须严格使用如下格式，单独成行，不要改写、不要加代码围栏：
   ...
   **三处分隔符必须使用 ASCII 半角冒号 \`:\`，禁止使用中文全角「：」作为分隔符**，否则程序无法解析。
3. 图表标题中不得出现任何冒号（半角或全角）。
4. 第四段为图表描述的中文自然语言文本，直接用中文写清描述，禁止使用 base64、URL 编码、引号转义等编码方式；描述中不得出现 \`%%\`、换行、反引号围栏；**描述正文内部**（非分隔符位置）如需出现冒号，请使用中文全角「：」以免与分隔符混淆。
5. 整个占位符必须写在同一行内，以 \`%%DIAGRAM:\` 开头，以 \`%%\` 结尾，中间不能换行；如果描述较长请浓缩在一行内写完。
6. 后续系统会根据语义自动选择合适的图表风格和类型（架构图、流程图、时序图等），你只需在描述中说明图表意图即可。
7. 图表描述必须具体说明图表包含哪些组件、分组、关系和关键连线约束。例如：描述一个三层架构图时需要说明每层包含哪些组件、层之间的数据流方向和协议。
8. 图表标题必须简洁清晰，图表描述必须具体到组件/阶段/数据流，不要写抽象词。
9. 占位符应紧跟在相关段落之后，不要集中堆到文末。
10. 绝对不要直接输出 \`\`\`mermaid 或 \`\`\`xml 代码块。程序只能识别 %%DIAGRAM%% 占位符，直接嵌入的代码块不会被正确渲染。`)
