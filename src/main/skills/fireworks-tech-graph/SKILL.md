...
### 1. `generate-diagram.js` - Validate SVG + export PNG

```bash
node ${CLAUDE_SKILL_DIR}/scripts/generate-diagram.js -t architecture -s 1 -o ./output/arch.svg
```

- Validates an existing SVG file
- Exports PNG after validation
- Example: `node ${CLAUDE_SKILL_DIR}/scripts/generate-diagram.js -t architecture -s 1 -o ./output/arch.svg`

### 2. `generate-from-template.js` - Create starter SVG from template

```bash
node ${CLAUDE_SKILL_DIR}/scripts/generate-from-template.js architecture ./output/arch.svg '{"title":"My Diagram","nodes":[],"arrows":[]}'
```
...
### 3. `validate-svg.js` - Validate SVG syntax

```bash
node ${CLAUDE_SKILL_DIR}/scripts/validate-svg.js <svg-file>
```
...
**MANDATORY: Array Method** (ALWAYS use this):
