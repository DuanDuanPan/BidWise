...
2. 每 3 轮 poll: 强制 re-read `../constitution.md` + `session-journal.yaml`
...
2. **Periodic refresh (每 3 轮):** Read `../constitution.md` + `session-journal.yaml`
...
4. **Poll each active story based on phase:**
...
5. **Update gate-state.yaml** with current story_states snapshot (通过 utility_pane)

6. **Inspector 主动监察 (每 3 轮, if inspector_state == idle):**
...
7. **Exit condition:** all stories in auto_qa_pending, uat_waiting, or done
   → **Read `./step-06-auto-qa-uat.md`**

8. **Continue condition:** any story still in dev, review, pending_review, or fixing
