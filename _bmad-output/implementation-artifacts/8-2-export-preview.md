...
- [x] 7.4 E2E
  ...
  - 场景至少覆盖：
    - [x] 点击预览按钮 → loading overlay 出现（或 error，取决于 docx bridge 可用性）
    - [x] `Cmd/Ctrl+E` 触发 preview
    - [x] 预览失败 → 错误态 + 重试 + 返回编辑
    - [x] Escape 关闭 error modal
    - [x] 取消加载中 → overlay 消失
    - [x] preview ready → 返回编辑（E2E mock: BIDWISE_E2E_EXPORT_PREVIEW_MOCK）
    - [x] 确认导出 → save dialog cancel 保持 modal → 再次确认导出成功 Toast（E2E mock: BIDWISE_E2E_EXPORT_DIALOG_PATH + CANCEL_COUNT）
