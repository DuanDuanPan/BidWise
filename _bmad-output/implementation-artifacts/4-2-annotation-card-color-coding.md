...
- [ ] Task 1: 定义批注五色常量与操作映射（AC: #1, #2, #5）
  - [ ] 1.1 新建 `src/renderer/src/modules/annotation/constants/annotation-colors.ts`
  - [ ] 1.2 定义五色映射常量：
    ```typescript
  ...
  - [ ] 1.3 定义每种类型的操作按钮配置：
    ...
    ```
  - [ ] 1.4 定义类型标签与图标映射（整合 Story 4.1 `AnnotationPanel.tsx` 中的 `TYPE_LABELS`）
  - [ ] 1.5 定义状态结果标签与颜色映射：
    ```typescript
    ...
    ```
  - [ ] 1.6 单测验证所有 `AnnotationType` 枚举值都有颜色、标签、图标和操作定义，并验证所有已处理 `AnnotationStatus` 都有结果标签与颜色定义

### 批注卡片组件

- [ ] Task 2: 创建 `AnnotationCard` 组件（AC: #1, #2, #3, #5, #6）
  - [ ] 2.1 新建 `src/renderer/src/modules/annotation/components/AnnotationCard.tsx`
  - [ ] 2.2 卡片布局结构：
  ...
  - [ ] 2.4 `cross-role` 类型：复用 `AnnotationHumanIcon`（暂无专属图标），文字标签区分为"跨角色"
  - [ ] 2.5 已处理状态（`accepted` / `rejected` / `needs-decision`）：
    - 整体 `opacity: 0.6`
    - 隐藏操作按钮组
  ...
  - [ ] 2.7 支持 `focused` prop 控制焦点态（蓝色 2px outline）
  - [ ] 2.8 支持 `ref` 转发，供键盘导航 `scrollIntoView` 使用
  - [ ] 2.9 使用 `data-testid="annotation-card"` 和 `data-annotation-id={annotation.id}`

### 批注面板升级

- [ ] Task 3: 升级 `AnnotationPanel` 集成五色卡片与键盘导航（AC: #4, #7）
  - [ ] 3.1 修改 `src/renderer/src/modules/project/components/AnnotationPanel.tsx`
  - [ ] 3.2 将 Story 4.1 的简化列表渲染替换为 `AnnotationCard` 组件
  - [ ] 3.3 将内联的 `TYPE_LABELS` / `TYPE_COLORS` / `STATUS_LABELS` / `STATUS_COLORS` 迁移到 Task 1 的集中常量文件，AnnotationPanel 引用常量文件
  - [ ] 3.4 实现键盘导航状态管理：
  ...
  - [ ] 3.6 键盘事件仅在标准展开面板或 compact flyout 打开且有批注时激活；当事件目标位于 `input` / `textarea` / `[contenteditable="true"]` / `[role="textbox"]` / `[data-testid="plate-editor-content"]` 内时不拦截，避免与编辑器快捷键冲突
  - [ ] 3.7 保留 Story 4.1 的壳层合同：
  ...
  - [ ] 3.8 更新 `src/renderer/src/modules/project/components/ProjectWorkspace.tsx` 如需传递新 props

### 测试

- [ ] Task 4: 单元测试、集成测试与 E2E（AC: #1-#7）
  - [ ] 4.1 `tests/unit/renderer/modules/annotation/constants/annotation-colors.test.ts` — 颜色映射完整性、操作定义完整性
  - [ ] 4.2 `tests/unit/renderer/modules/annotation/components/AnnotationCard.test.tsx` — 五色渲染、三重编码、操作按钮按类型渲染、点击状态变更、placeholder 提示、已处理降低透明度、状态标签、焦点态、`aria-label` 摘要
  - [ ] 4.3 `tests/unit/renderer/project/AnnotationPanel.test.tsx` — 升级后的卡片列表渲染、键盘导航（Alt+↑/↓ 循环）、快捷操作（Alt+Enter/Alt+Backspace/Alt+D）、无 reject 操作 no-op、已处理卡片 no-op、编辑器目标不拦截、壳层合同保留
  - [ ] 4.4 `tests/e2e/stories/story-4-2-annotation-card-color.spec.ts` — 预置不同类型批注 → 验证五色左边框 → 操作按钮文案 → 键盘导航 → 状态变更后卡片降低透明度
  - [ ] 4.5 `pnpm test`, `pnpm lint`, `pnpm typecheck`, `pnpm build` 全部通过
