...
- [ ] **Task 1: Python 项目初始化** (AC: #1, #6)
  ...
  - [ ] 1.4 更新项目根目录 `.gitignore` 添加 Python 条目
    - 新增：`__pycache__/`, `*.pyc`, `*.pyo`, `.venv/`, `python/.venv/`, `*.egg-info/`
    - `dist/` 已存在时不得重复添加

- [ ] **Task 2: FastAPI 应用与启动入口** (AC: #1, #3)
  - [ ] 2.1 实现 `docx_renderer/app.py`
...
- [ ] **Task 2.3: Shutdown 端点** (AC: #7)
  - [ ] 实现 `routes/shutdown.py`
...
- [ ] **Task 5: 基础渲染引擎** (AC: #4, #5)
  - [ ] 5.1 实现 `engine/renderer.py`
...
- [ ] **Task 6: Node.js 侧共享类型定义** (AC: #8)
  ...
  - [ ] 6.2 在 `src/shared/ipc-types.ts` 添加 IPC 通道
  ...
  - [ ] 6.3 在 `src/shared/constants.ts` 的 `ErrorCode` 枚举中添加
  ...
  - [ ] 6.4 在 `src/main/utils/errors.ts` 添加 `DocxBridgeError extends BidWiseError`
...
- [ ] **Task 7: Python 进程管理器** (AC: #1, #2, #7)
  - [ ] 7.1 创建 `src/main/services/docx-bridge/process-manager.ts`
...
- [ ] **Task 8: HTTP 渲染客户端** (AC: #4, #8)
  - [ ] 8.1 创建 `src/main/services/docx-bridge/render-client.ts`
...
- [ ] **Task 9: docx-bridge 服务门面** (AC: #8)
  - [ ] 9.1 创建 `src/main/services/docx-bridge/index.ts`
...
- [ ] **Task 10: IPC 处理器与注册** (AC: #8)
  - [ ] 10.1 创建 `src/main/ipc/docx-bridge-handlers.ts`
...
- [ ] **Task 12: Python 侧测试** (AC: #9)
  - [ ] 12.1 `tests/conftest.py` — FastAPI TestClient fixture，无需实际启动 uvicorn
  - [ ] 12.2 `tests/test_health.py` — health 端点返回正确格式、版本号、camelCase `uptimeSeconds`
  - [ ] 12.3 `tests/test_render.py` — 基础渲染：标题/段落/列表/表格，模板加载，模板缺失错误，模板无效错误，空内容处理，camelCase 请求/响应字段
  - [ ] 12.4 `tests/test_engine.py` — renderer 核心逻辑单元测试，docx 文件验证（用 python-docx 打开验证内容）
  - [ ] 12.5 `tests/test_shutdown.py` — shutdown 端点返回 `{ success: true, data: { accepted: true } }`
  - [ ] 12.6 测试数据：在 `python/tests/fixtures/` 放置测试用 Markdown 文件和 `.docx` 模板文件

- [ ] **Task 13: Node.js 侧测试** (AC: #1, #2, #7, #8)
  - [ ] 13.1 `tests/unit/main/services/docx-bridge-process-manager.test.ts` — 进程启动/停止/重启/超时/READY 解析、最多 3 次重试、`cwd`/`PYTHONPATH`、`--host 127.0.0.1 --port 0`
  - [ ] 13.2 `tests/unit/main/services/docx-bridge-render-client.test.ts` — HTTP 请求/响应/超时/错误处理、camelCase payload
  - [ ] 13.3 `tests/unit/main/ipc/docx-bridge-handlers.test.ts` — IPC handler 薄分发
  - [ ] 13.4 `tests/unit/preload/security.test.ts` — 安全白名单包含 docx 通道（更新现有文件）
  - [ ] 13.5 `tests/unit/main/services/docx-bridge-service.test.ts` — `outputPath` 必须限制在项目 `exports/` 下，路径逃逸返回 `VALIDATION`
  - [ ] 13.6 `tests/unit/main/index.test.ts` 或现有启动测试扩展 — docxBridge 以后台方式启动，不阻塞 `createWindow()`
  - [ ] 13.7 `tests/integration/docx-bridge/bridge-integration.test.ts` — 启动真实 Python 进程，发送渲染请求，验证 READY 回报实际端口、shutdown、docx 输出
  - [ ] 13.8 验证命令全部通过：`pnpm test && pnpm lint && pnpm typecheck && pnpm build` 以及 `cd python && pytest`
