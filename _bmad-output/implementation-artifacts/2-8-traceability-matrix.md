...
- [x] 1.1 在 `src/main/db/schema.ts` 中新增 `TraceabilityLinkTable` 接口
...
- [x] 1.2 创建迁移文件 `src/main/db/migrations/007_create_traceability_links.ts`
...
- [x] 1.3 创建 `src/main/db/repositories/traceability-link-repo.ts`
...
- [x] 1.4 在 `src/main/db/migrator.ts` 中显式注册 `007_create_traceability_links`

### Task 2: 共享契约 — `proposal.meta.json` 章节索引 + Traceability 类型 (AC: #1, #2, #3, #4, #5)

- [x] 2.1 在 `src/shared/template-types.ts` 中新增 `ProposalSectionIndexEntry`
...
- [x] 2.2 在 `src/shared/models/proposal.ts` 中扩展 `ProposalMetadata`
  - 新增 `sectionIndex?: ProposalSectionIndexEntry[]`
  - 保留既有 `sectionWeights` / `templateId`，但追溯矩阵列定义以后续 `sectionIndex` 为准
- [x] 2.3 在 `src/main/services/document-service.ts` 与 `src/main/services/template-service.ts` 中同步扩展 `proposal.meta.json` 的读写
  - Story 3.3 生成/持久化骨架时，必须同时写入完整 `sectionIndex`
  - 兼容旧项目：`sectionIndex` 缺失时不报错，由 Story 2.8 在读取 `proposal.md` 时兜底合成临时索引
- [x] 2.4 在 `src/shared/analysis-types.ts` 末尾新增 `// ─── Story 2.8: Traceability Matrix ───` 区块
...
- [x] 2.5 在 `src/shared/constants.ts` 中新增 `ErrorCode.MATRIX_GENERATION_FAILED`、`ErrorCode.ADDENDUM_PARSE_FAILED`
- [x] 2.6 在 `src/shared/ipc-types.ts` 中新增频道与映射
...
- [x] 2.7 在 `src/shared/ai-types.ts` 中将 `AgentType` 扩展新增 `'traceability'`

### Task 3: Prompt 层 — 追溯映射与补遗提取 (AC: #1, #3)

- [x] 3.1 创建 `src/main/prompts/generate-traceability.prompt.ts`
...
- [x] 3.2 创建 `src/main/prompts/extract-addendum.prompt.ts`
  - 输出与需求抽取兼容的 requirement 条目，只提取补遗中新出现或被实质变更的要求
  - 明确这是“补遗/变更通知”语境，不重复回传未变化的原始需求
- [x] 3.3 在 `src/main/prompts/index.ts` 中导出上述新 prompt

### Task 4: Agent 编排 — `traceability-agent` + extract addendum mode (AC: #1, #3)

- [x] 4.1 创建 `src/main/services/agent-orchestrator/agents/traceability-agent.ts`
...
- [x] 4.2 在 `src/main/services/agent-orchestrator/index.ts` 的真实注册入口注册 `traceability`
- [x] 4.3 扩展 `src/main/services/agent-orchestrator/agents/extract-agent.ts`
...
- [x] 5.1 创建 `src/main/services/document-parser/traceability-matrix-service.ts`
...
- [x] 5.2 `generate(input: GenerateMatrixInput)` 方法
...
- [x] 5.6 `importAddendum(input: ImportAddendumInput)` 方法
...
- [x] 5.7 在 `src/main/services/document-parser/index.ts` 导出单例：`export const traceabilityMatrixService = new TraceabilityMatrixService()`

### Task 6: IPC / preload 接线 (AC: #1, #3, #4)

- [x] 6.1 在 `src/main/ipc/analysis-handlers.ts` 中注册 7 个新频道
  - `analysis:generate-matrix`
  - `analysis:get-matrix`
  - `analysis:get-matrix-stats`
  - `analysis:create-link`
...
- [x] 6.2 在 `src/preload/index.ts` 与 `src/preload/index.d.ts` 中暴露并声明对应 API

### Task 7: Renderer 状态与任务监控 (AC: #1, #2, #3, #4, #5, #6)

- [x] 7.1 在 `src/renderer/src/stores/analysisStore.ts` 的 `AnalysisProjectState` 中新增字段
...
- [x] 7.4 扩展 `src/renderer/src/modules/analysis/hooks/useAnalysis.ts`
  - `TaskKind` / task monitor 识别 `'matrix' | 'addendum'`
  - 与现有 `useAnalysisTaskMonitor` 模式保持一致，进度消息回写 store

### Task 8: UI 组件与跨阶段导航 (AC: #2, #3, #4, #6)

- [x] 8.1 创建 `src/renderer/src/modules/analysis/components/TraceabilityMatrixView.tsx`
...
- [x] 8.2 创建 `src/renderer/src/modules/analysis/components/ComplianceCoverageMatrix.tsx`
  - 评分族视觉风格，对齐 UX-DR14
...
- [x] 8.3 创建 `src/renderer/src/modules/analysis/components/AddendumImportModal.tsx`
...
- [x] 8.4 在 `src/renderer/src/modules/analysis/components/AnalysisView.tsx` 中新增「追溯矩阵」Tab
  - 位于「策略种子」Tab 之后
  - Tab 始终可点击进入，以便展示前置条件说明
  - 当前置条件缺失时使用 muted 样式 + tooltip，而不是彻底 disabled
  - Badge 显示未覆盖数 / 覆盖率 / 全绿状态
- [x] 8.5 在 `src/renderer/src/modules/project/components/ProjectWorkspace.tsx`（或同级 workspace 状态承载点）实现跨阶段章节跳转桥接
...
- [x] 9.1 `tests/unit/main/db/repositories/traceability-link-repo.test.ts`
  - `replaceAutoByProject()` 保留 manual
  - 唯一约束
  - 需求删除级联清理
  - `matchReason` 持久化
- [x] 9.2 `tests/unit/main/services/document-parser/traceability-matrix-service.test.ts`
...
- TypeScript compilation: EXIT:0 (clean)
- Test suite: 108 files passed, 885 tests passed; 5 pre-existing failures (better-sqlite3 native module)
- Lint: all errors fixed, formatting applied via prettier

### Completion Notes List

- ✅ Task 1: Data layer — `TraceabilityLinkTable` schema, migration 007, repository with `replaceAutoByProject` transaction pattern
- ✅ Task 2: Shared types — 15+ new types in analysis-types.ts, ProposalSectionIndexEntry, 7 IPC channels, 2 error codes, AgentType extended
- ✅ Task 3: Prompt layer — `generate-traceability.prompt.ts` and `extract-addendum.prompt.ts` with structured JSON output
- ✅ Task 4: Agent orchestration — `traceability-agent` (temp 0.2), extract-agent extended with `addendum-requirements` mode
- ✅ Task 5: Backend service — `TraceabilityMatrixService` with generate, getMatrix, getStats, CRUD, importAddendum, snapshot persistence
- ✅ Task 6: IPC/preload — 7 new analysis handlers registered, preload API methods exposed
- ✅ Task 7: Renderer state — analysisStore extended with traceability/addendum fields and actions, task monitor updated
- ✅ Task 8: UI components — TraceabilityMatrixView, ComplianceCoverageMatrix, AddendumImportModal; AnalysisView tab; cross-stage chapter navigation bridge in ProjectWorkspace
- ✅ Task 9: Tests — 42+ new tests across repo, service, prompt, agent, store layers; 2 existing tests updated for new fields

### Change Log

- 2026-04-03: Full implementation of Story 2.8 — Requirement-Proposal Bidirectional Traceability Matrix

### File List

**New files:**
- src/main/db/migrations/007_create_traceability_links.ts
- src/main/db/repositories/traceability-link-repo.ts
- src/main/prompts/generate-traceability.prompt.ts
- src/main/prompts/extract-addendum.prompt.ts
- src/main/services/agent-orchestrator/agents/traceability-agent.ts
...
- src/renderer/src/modules/analysis/components/AddendumImportModal.tsx
- tests/unit/main/db/repositories/traceability-link-repo.test.ts
- tests/unit/main/services/document-parser/traceability-matrix-service.test.ts
- tests/unit/main/prompts/generate-traceability.prompt.test.ts
- tests/unit/main/prompts/extract-addendum.prompt.test.ts
- tests/unit/main/services/agent-orchestrator/agents/traceability-agent.test.ts
- tests/unit/renderer/stores/analysisStore.traceability.test.ts
