...
- [ ] Task 1: 共享 markdown 结构变更 helper（AC: 1, 4, 8）
  - [ ] 1.1 扩展 `src/shared/chapter-markdown.ts`：新增“按 section subtree 插入 / 缩进 / 反缩进”的纯函数 helper，能够搬运整段 heading block，而不是只替换正文
  - [ ] 1.2 helper 需要同时处理目标节点及其全部后代的 heading level 调整，保证 `Tab` / `Shift+Tab` 后整棵子树层级一致
  - [ ] 1.3 `Enter` 新增同级节点时沿用 Story 3.3 先例，默认标题使用 `新章节`，确保 markdown heading 与 outline 解析始终有效
  - [ ] 1.4 为 shared helper 补充重复标题、嵌套子树、根节点边界、H4 子树缩进后的层级修正测试

- [ ] Task 2: 扩展 chapter-structure main service 与 IPC mutation contract（AC: 1, 4, 7, 8）
  - [ ] 2.1 扩展 `src/main/services/chapter-structure-service.ts`：新增 `insertSibling(projectId, sectionId)`、`indent(projectId, sectionId)`、`outdent(projectId, sectionId)` 等 mutation
  - [ ] 2.2 每个 mutation 内部读取当前 `proposal.md` + `proposal.meta.json.sectionIndex`，完成 markdown subtree 变更、`headingLocator` 重算、`parentSectionId/order/occurrenceIndex` 归一化，并复用 `normalizeSiblingOrder()`
  - [ ] 2.3 mutation 输出统一返回最新 snapshot，例如：
    ```typescript
    {
      markdown: string
    ...
    ```
  - [ ] 2.4 扩展 `src/shared/ipc-types.ts`、`src/main/ipc/chapter-structure-handlers.ts`、`src/preload/index.ts`：新增 `chapter-structure:insert-sibling`、`chapter-structure:indent`、`chapter-structure:outdent`
  - [ ] 2.5 IPC handler 保持 thin-wrapper 形态，业务逻辑全部留在 `chapter-structure-service`

- [ ] Task 3: renderer 侧 `chapterStructureStore` 动作扩展（AC: 1, 5, 7, 8）
  - [ ] 3.1 在 Story 11.2 的 `src/renderer/src/stores/chapterStructureStore.ts` 上扩展 actions：`insertSibling(projectId, nodeKey)`、`indentNode(projectId, nodeKey)`、`outdentNode(projectId, nodeKey)`
  - [ ] 3.2 store 通过 `sectionIdByNodeKey[nodeKey]` 解析 canonical `sectionId`，调用对应 IPC mutation，并把返回的 `markdown` / `sectionIndex` 同步写回 `useDocumentStore`
  - [ ] 3.3 mutation 成功后刷新 outline metadata，调用 `focusNode(nextNodeKey)`，并在 `Enter` 新建节点场景下继续调用 `enterEditing(nextNodeKey)`
  - [ ] 3.4 mutation 成功后按返回 snapshot 计算最新深度；深度超过 6 时触发 Story 11.5 的 `warnDepthExceeded(depth)`
  - [ ] 3.5 `locked` / `pending-delete` / `editing` 状态下，store 直接返回拒绝结果，避免组件各自散落 guard

- [ ] Task 4: `useStructureKeymap` 与树导航（AC: 1, 2, 3, 4, 5, 6）
  - [ ] 4.1 创建 `src/renderer/src/modules/editor/hooks/useStructureKeymap.ts`
    ```typescript
    ...
    ```
  - [ ] 4.2 hook 将按键监听绑定到 `panelRef.current` 对应的 focusable tree root，统一处理 `Enter` / `Tab` / `Shift+Tab` / `Delete` / `F2` / `Esc` / 方向键，并在命中时 `preventDefault()`
  - [ ] 4.3 方向键导航通过当前 `outline` 计算“可见节点顺序”和父子关系，复用 `onNavigateToNode()`，保持与鼠标点击节点相同的 `scrollToHeading()` 行为
  - [ ] 4.4 `Delete` / `Backspace` 只在树根获焦且当前节点可操作时触发；focus 位于 inline Input、按钮、Toast Undo 按钮时保留各自控件的原生键盘语义
  - [ ] 4.5 `Enter` 在 Focused 态下创建同级节点，在 Editing 态下沿用 Story 11.2 的提交语义

- [ ] Task 5: 集成到现有结构面板链路（AC: 1, 2, 3）
  - [ ] 5.1 更新 `src/renderer/src/modules/editor/components/DocumentOutlineTree.tsx`：根节点新增 `ref`、`tabIndex={0}`、focus outline 样式，并挂载 `useStructureKeymap()`
  - [ ] 5.2 更新 `DocumentOutlineTreeNode.tsx`（Story 11.2）或同等 title slot 组件：双击进入 Editing、inline Input 的键盘事件停止冒泡到 tree root
  - [ ] 5.3 更新 `src/renderer/src/modules/project/components/ProjectWorkspace.tsx`：抽出一个统一的 `handleOutlineNavigate(node)`，同时供鼠标点击与方向键导航复用
  - [ ] 5.4 `OutlinePanel.tsx` 继续保持 240px / 40px 壳层合同，本 Story 的快捷键集成点落在其内部的 `DocumentOutlineTree`

- [ ] Task 6: 删除交接与测试矩阵（AC: 5, 6, 7, 8）
  - [ ] 6.1 预留 Story 11.4 删除入口：`requestSoftDelete(projectId, sectionIds, nodeKeys)` 或等价 action，11.3 负责在按键发生时传入完整的级联目标集合
  - [ ] 6.2 新建 `tests/unit/shared/chapter-markdown-structure.test.ts`：覆盖插入 sibling、Tab 缩进、Shift+Tab 反缩进、重复标题、子树级联搬运
  - [ ] 6.3 新建 `tests/unit/main/services/chapter-structure-service.test.ts`：覆盖 snapshot 返回、`sectionId` 稳定性、`headingLocator` 重算、边界幂等
  - [ ] 6.4 新建 `tests/unit/renderer/modules/editor/hooks/useStructureKeymap.test.ts`：覆盖作用域启停、Editing guard、Locked guard、方向键导航、Delete 目标集合构造
  - [ ] 6.5 更新 `tests/unit/renderer/modules/editor/components/DocumentOutlineTree.test.tsx`：覆盖 `tabIndex`、树根 focus、键盘导航后仍保持受控焦点态
  - [ ] 6.6 Playwright E2E：完整验证“纯键盘创建 5 层 10 节点结构、缩进 / 反缩进、删除后进入 11.4 Undo 流程、深度警告出现”
