...
- [x] Task 1: 三栏布局容器组件 (AC: 1)
  - [x] 1.1 创建 `src/renderer/src/modules/project/components/WorkspaceLayout.tsx` — 三栏布局容器
  ...
  - [x] 1.2 修改 `ProjectWorkspace.tsx` — 将当前单栏布局替换为 `<WorkspaceLayout>`
...
- [x] Task 2: 左侧大纲面板占位 (AC: 1)
  - [x] 2.1 创建 `src/renderer/src/modules/project/components/OutlinePanel.tsx` — 左侧面板壳子
  ...
  - [x] 2.2 折叠/展开受 `useWorkspaceLayout` hook 控制（见 Task 4）

- [x] Task 3: 右侧批注面板占位 (AC: 1)
  - [x] 3.1 创建 `src/renderer/src/modules/project/components/AnnotationPanel.tsx` — 右侧面板壳子
  ...
  - [x] 3.2 折叠/展开受 `useWorkspaceLayout` hook 控制（见 Task 4）

- [x] Task 4: 面板状态管理 hook (AC: 1, 2, 3)
  - [x] 4.1 创建 `src/renderer/src/modules/project/hooks/useWorkspaceLayout.ts`
...
- [x] Task 5: 面板快捷键 (AC: 2)
  - [x] 5.1 创建 `src/renderer/src/modules/project/hooks/useWorkspaceKeyboard.ts`
  ...
  - [x] 5.2 在 `ProjectWorkspace` 中挂载此 hook，仅在 `/project/:id` 路由下激活

- [x] Task 6: 底部状态栏 (AC: 1)
  - [x] 6.1 创建 `src/renderer/src/modules/project/components/StatusBar.tsx`
...
- [x] Task 7: 主内容区 800px 限宽容器 (AC: 4)
  - [x] 7.1 在 `WorkspaceLayout` 的 center slot 内创建限宽包装：
...
- [x] Task 8: 单元测试 (AC: 全部)
  - [x] 8.1 `WorkspaceLayout` 组件测试：三栏正确渲染、面板折叠/展开切换、折叠动画类名切换
  - [x] 8.2 `OutlinePanel` 组件测试：渲染标题和占位内容、折叠按钮触发回调
  - [x] 8.3 `AnnotationPanel` 组件测试：渲染标题和 Badge、折叠按钮触发回调、紧凑模式图标栏渲染、紧凑模式点击图标打开 flyout、flyout 外点击关闭、Escape 关闭 flyout
  - [x] 8.4 `StatusBar` 组件测试：三个占位指标正确渲染、SOP 阶段名称显示
  - [x] 8.5 `useWorkspaceLayout` hook 测试：初始状态、toggleOutline/toggleSidebar、resize 触发紧凑模式、手动覆盖逻辑
  - [x] 8.6 `useWorkspaceKeyboard` hook 测试：Cmd/Ctrl+B 触发 toggleSidebar、Cmd/Ctrl+\ 触发 toggleOutline
  - [x] 8.7 `ProjectWorkspace` 集成测试：三栏布局渲染、面板折叠后主内容区宽度正确扩展
  - [x] 8.8 无障碍测试：面板折叠按钮 `aria-expanded` 切换、侧栏 `role="complementary"` 存在、状态栏 `role="status"` 存在、键盘 Tab 可遍历所有可交互元素

- [x] Task 9: 集成验证 (AC: 全部)
  - [x] 9.1 验证全链路：看板卡片 → 进入工作空间 → 三栏布局 + SOP 进度条 + 状态栏均正确渲染
  - [x] 9.2 验证面板折叠：Cmd/Ctrl+B 和 Cmd/Ctrl+\ 正确切换对应面板
  - [x] 9.3 验证紧凑模式：手动调整窗口宽度 <1440px → 两侧面板自动折叠；手动展开后不被自动策略覆盖
  - [x] 9.4 验证内容限宽：主内容区内容不超过 800px，两侧留白自然吸收
  - [x] 9.5 验证 `lint && typecheck && build` 全部通过
  - [x] 9.6 验证与 Story 1.6 功能无回归：SOP 导航、阶段跳转、快捷键、状态持久化均正常
...
- jsdom window.innerWidth defaults to 0 → compact mode triggers in integration tests. Fixed by setting window.innerWidth=1600 in beforeEach.

### Completion Notes List

- ✅ Task 1: Created `WorkspaceLayout.tsx` — flexbox 3-column container with left/center/right/statusBar slots. Modified `ProjectWorkspace.tsx` to use new layout, replacing single-column div.
- ✅ Task 2: Created `OutlinePanel.tsx` — 240px left panel with title bar, fold/unfold toggle, placeholder text. Width transitions via CSS `--duration-panel` + `--ease-in-out`. Collapses to 0 width.
- ✅ Task 3: Created `AnnotationPanel.tsx` — 320px right panel with two modes: (a) standard mode with full panel, (b) compact mode (<1440px) with 48px icon bar + flyout overlay. Flyout has `role="dialog"`, closes on Escape/outside click, focus management.
- ✅ Task 4: Created `useWorkspaceLayout.ts` — panel collapse state + responsive compact mode detection. 200ms throttled resize listener, auto-collapse <1440px, manualOverride flags reset on breakpoint crossing.
- ✅ Task 5: Created `useWorkspaceKeyboard.ts` — Cmd/Ctrl+B toggles sidebar, Cmd/Ctrl+\\ toggles outline. Mounted in ProjectWorkspace.
- ✅ Task 6: Created `StatusBar.tsx` — 32px bottom bar with 3 placeholder metrics (合规/质量/字数) + current SOP stage name. `role="status"`.
- ✅ Task 7: 800px max-width wrapper in WorkspaceLayout center slot with `overflow-x: auto` for wide tables.
- ✅ Task 8: 45 new unit tests across 6 test files + 7 integration tests in ProjectWorkspace.test.tsx. Covers all components, hooks, ARIA attributes, flyout behavior, keyboard shortcuts, responsive breakpoints.
- ✅ Task 9: All 158 tests pass. lint, typecheck pass. No regressions in Story 1.6 (SOP navigation, stage jumping, Alt+2~6 shortcuts, stage persistence all verified by existing tests).
- ✅ Added `@keyframes flyout-slide-in` + `prefers-reduced-motion` override to globals.css.
- ✅ Updated barrel exports in `modules/project/index.ts`.

### Change Log

- 2026-03-21: Story 1-7 complete — 三栏布局壳子 (WorkspaceLayout + OutlinePanel + AnnotationPanel + StatusBar + useWorkspaceLayout + useWorkspaceKeyboard)
...
- `tests/unit/renderer/project/WorkspaceLayout.test.tsx`
...
- `tests/unit/renderer/project/useWorkspaceLayout.test.ts`
- `tests/unit/renderer/project/useWorkspaceKeyboard.test.ts`

**修改文件：**
- `src/renderer/src/modules/project/components/ProjectWorkspace.tsx` — 替换单栏布局为三栏 WorkspaceLayout
- `src/renderer/src/modules/project/index.ts` — 添加新组件/hook 导出
- `src/renderer/src/globals.css` — 添加 flyout-slide-in 动画 + prefers-reduced-motion 降级
- `tests/unit/renderer/project/ProjectWorkspace.test.tsx` — 添加 Story 1-7 集成测试
