...
- [ ] Task 2: 脱敏服务 desensitizer.ts (AC: 1, 2)
  ...
  - [ ] 2.2 实现 `Desensitizer` 类：`desensitize(text: string): DesensitizeResult` 和 `restore(text: string, mappingId: string): string`
  ...
  - [ ] 2.7 导出 `DesensitizeResult` 类型：`{ text: string; mappingId: string; stats: { totalReplacements: number; byType: Record<string, number> } }`
...
- [ ] Task 4: AI Proxy 服务入口 index.ts (AC: 1, 2, 3, 4, 5)
  ...
  - [ ] 4.5 aiProxy 初始化时通过 `src/main/config/app-config.ts` 统一读取 Provider 配置（禁止在 ai-proxy/index.ts 中直接读取环境变量或自行实现配置加载）；Alpha 阶段在 app-config.ts 中新增 `getAiProxyConfig(): AiProxyConfig`，底层从环境变量读取 `AI_PROVIDER`、`ANTHROPIC_API_KEY`、`OPENAI_API_KEY`
  - [ ] 4.6 配置类型 `AiProxyConfig`：`{ provider: 'claude' | 'openai', anthropicApiKey?, openaiApiKey?, defaultModel?, desensitizeEnabled: boolean }`（类型定义在 `src/shared/ai-types.ts`，配置读取在 `app-config.ts`）
...
- [ ] Task 5: AI 调用链日志 (AC: 5)
  ...
  - [ ] 5.3 日志条目结构：`{ timestamp, caller, provider, model, desensitizedInput: messages[], outputContent: string, inputTokens, outputTokens, latencyMs, status: 'success'|'error', errorCode?, desensitizeStats }`（`desensitizedInput` 和 `outputContent` 是本次修复新增的必填字段）
  - [ ] 5.4 `desensitizedInput` 记录脱敏后的完整 messages，`outputContent` 记录 AI 原始返回内容（此时尚未还原，内容本身不含敏感数据）——永不记录还原后的原文
...
- [ ] Task 10: ensureDataDirectories 异步化 + 补充 desensitize-mappings 目录 (AC: 2)
  - [ ] 10.1 将 `src/main/index.ts` 中 `ensureDataDirectories()` 的 `mkdirSync` 调用全部替换为 `await fs.promises.mkdir(path, { recursive: true })`，函数签名改为 `async ensureDataDirectories(): Promise<void>`，调用处使用 `await`
  - [ ] 10.2 在 `ensureDataDirectories()` 的子目录列表中新增 `desensitize-mappings`（确保 `data/desensitize-mappings/` 目录在应用启动时创建）
  - [ ] 10.3 验证所有调用 `ensureDataDirectories()` 的位置已适配 async/await（避免目录尚未创建就被后续代码使用）
  - **Why**: codex 第 2 轮验证唯一剩余 FAIL——Story 反模式清单明确禁止同步 I/O（`mkdirSync`），且 `data/desensitize-mappings/` 缺失会导致 mapping-store 运行时写入失败

- [ ] Task 9: 集成验证 (AC: 全部)
  - [ ] 9.1 验证 `pnpm lint && pnpm typecheck && pnpm build` 全部通过
  - [ ] 9.2 验证新增依赖不影响 Electron 打包（无 native module 冲突）
  - [ ] 9.3 验证冷启动时间仍 <5 秒
...
- `src/main/config/app-config.ts` — 新增 `getAiProxyConfig()` 配置读取
- `src/main/index.ts` — `ensureDataDirectories()` 的 subdirs 中补充 `desensitize-mappings`（如尚未包含）
...
- 2026-03-20 — 基于 codex 验证修复：(1) 消除重复造轮子——Task 5.5 复用 ensureDataDirectories、Task 4.5 通过 app-config.ts 读取配置、UUID 复用 project-repo 模式；(2) AC2 映射表持久化改为必选；(3) AC5 日志新增 desensitizedInput/outputContent 字段；(4) 核心流程安全时序修正——traceLog 在 restore 之前；(5) 新增 mapping-store.ts 持久化任务、app-config.ts 扩展任务、可重试/不可重试错误测试；(6) 禁止同步 I/O 反模式
