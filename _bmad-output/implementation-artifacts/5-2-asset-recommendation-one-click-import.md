...
- [x] 1.1 在 `src/shared/asset-types.ts` 新增 `CreateAssetInput` 类型
...
- [x] 1.2 在 `src/main/db/repositories/asset-repo.ts` 新增 `create()` 方法
...
- [x] 1.3 在 `src/main/services/asset-service.ts` 新增 `create()` 方法
...
- [x] 1.4 在 `src/shared/ipc-types.ts` 新增 `asset:create` 频道
  - `IPC_CHANNELS` 新增 `ASSET_CREATE: 'asset:create'`
  - `IpcChannelMap` 新增 `'asset:create': { input: CreateAssetInput; output: AssetDetail }`
- [x] 1.5 在 `src/main/ipc/asset-handlers.ts` 注册 `asset:create` handler
  - 使用 `createIpcHandler()` 模式，handler 仅做参数透传
- [x] 1.6 确认 `src/main/ipc/index.ts` **无需**新增新的注册调用
...
- [x] 1.7 更新 `src/preload/index.ts` 暴露 `window.api.assetCreate()`
- [x] 1.8 更新 `src/preload/index.d.ts` 类型声明
- [x] 1.9 在 `src/renderer/src/stores/assetStore.ts` 新增 `createAsset(input: CreateAssetInput): Promise<void>` action
...
- [x] 2.1 创建 `src/shared/recommendation-types.ts`
...
- [x] 2.2 创建 `src/main/services/recommendation-service.ts`
...
- [x] 2.3 在 `src/shared/ipc-types.ts` 新增 `asset:recommend` 频道
...
- [x] 2.4 在 `src/main/ipc/asset-handlers.ts` 注册 `asset:recommend` handler
- [x] 2.5 确认 `src/main/ipc/index.ts` 现有 `registerAssetHandlers()` 注册位保持不变
- [x] 2.6 更新 `src/preload/index.ts` 暴露 `window.api.assetRecommend()`
- [x] 2.7 更新 `src/preload/index.d.ts`

### Task 3: 推荐状态管理 (AC: #1, #3, #4, #5, #6)

- [x] 3.1 创建 `src/renderer/src/stores/recommendationStore.ts`
  - State：
  ...
  - 约束：
    - `loading: boolean` 命名
    - 推荐数据为内存态，不持久化
- [x] 3.2 在 `src/renderer/src/stores/index.ts` 导出 `useRecommendationStore`
- [x] 3.3 创建 `src/renderer/src/modules/asset/hooks/useAssetRecommendation.ts`
...
- [x] 4.1 创建 `src/renderer/src/modules/asset/components/RecommendationPanel.tsx`
  ...
  - 加载中显示轻量 Spin（不阻塞编辑器）
  - 可折叠/展开
- [x] 4.2 创建 `src/renderer/src/modules/asset/components/RecommendationCard.tsx`
...
- [x] 4.3 创建 `src/renderer/src/modules/asset/components/RecommendationDetailDrawer.tsx`
  - Ant Design `Drawer` 从右侧滑出
  - 宽度 `480px`
  - 展示：标题、资产类型标签、来源项目、可选来源章节、匹配度、全部标签、完整正文内容
  - 底部操作：`插入到编辑器`（primary）、`关闭`
- [x] 4.4 在 `RecommendationPanel` / `RecommendationDetailDrawer` 中复用现有 `window.api.assetGet({ id })`
...
- [x] 5.1 创建 `src/renderer/src/modules/asset/components/AssetImportDialog.tsx`
...
- [x] 5.2 创建 `src/renderer/src/modules/asset/hooks/useAssetImport.ts`
  - 管理对话框 open/close 状态
...
- [x] 5.3 在编辑器工具栏添加"一键入库"按钮
  ...
  - Tooltip：`将选中片段保存到资产库`
- [x] 5.4 更新 `src/renderer/src/modules/editor/components/EditorView.tsx`
...
- [x] 6.2 更新 `src/renderer/src/modules/editor/components/EditorView.tsx`
...
- [x] 7.1 将 `RecommendationPanel` 集成到项目工作空间侧边栏
...
- [x] 8.1 更新 `tests/unit/main/db/repositories/asset-repo.test.ts`
  - 新增 `create()` 测试：默认 summary 截取、FTS 触发器同步验证（创建后可搜到）、必填字段校验
- [x] 8.2 更新 `tests/unit/main/services/asset-service.test.ts`
  - 新增 `create()` 测试：标签创建与映射、sourceProject/sourceSection 填充、日志记录
- [x] 8.3 新建 `tests/unit/main/services/recommendation-service.test.ts`
  - 覆盖：
...
- [x] 8.4 更新 `tests/unit/main/ipc/asset-handlers.test.ts`
  - 新增 `asset:create` 和 `asset:recommend` 频道注册、透传与错误包装测试
- [x] 8.5 更新 `tests/unit/preload/security.test.ts`
  - 新增 `assetCreate` 和 `assetRecommend` 到 preload 白名单断言
- [x] 8.6 新建 `tests/unit/renderer/stores/recommendationStore.test.ts`
  - 覆盖：获取推荐、忽略（从列表移除）、采纳（保留但标记）、同章节刷新时已忽略项不回流、同章节刷新时已插入项保持 accepted、章节切换清空、loading/error 状态、过期响应丢弃
- [x] 8.7 新建 `tests/unit/renderer/modules/asset/components/RecommendationCard.test.tsx`
  - 覆盖：绿色样式渲染、三个操作按钮点击回调、标签溢出 `+N`、已采纳态视觉变化
- [x] 8.8 新建 `tests/unit/renderer/modules/asset/components/RecommendationPanel.test.tsx`
  - 覆盖：asset:get 详情加载、Drawer 打开/关闭、插入成功后标记已插入、忽略后列表移除
- [x] 8.9 新建 `tests/unit/renderer/modules/asset/components/AssetImportDialog.test.tsx`
  - 覆盖：预填内容正确、标签编辑交互、提交成功后关闭并显示 Toast、取消关闭、必填校验
- [x] 8.10 更新 `tests/unit/renderer/modules/editor/components/EditorToolbar.test.tsx`
  - 覆盖："一键入库"按钮渲染、仅编辑器内选区可 enabled、非编辑器选区保持 disabled、点击回调
- [x] 8.11 更新 `tests/unit/renderer/modules/editor/components/EditorView.test.tsx`
  - 覆盖：`currentSection` 传入后可打开导入对话框、`onInsertAssetReady` 向上传递
- [x] 8.12 更新 `tests/unit/renderer/modules/editor/components/PlateEditor.test.tsx`
  - 覆盖：`onInsertAssetReady` 注册、selection / lastSelection / section fallback / 文档末尾 fallback
- [x] 8.13 更新 `tests/unit/renderer/project/AnnotationPanel.test.tsx` 与 `tests/unit/renderer/project/ProjectWorkspace.test.tsx`
...
- [x] 8.14 新建 `tests/e2e/stories/story-5-2-asset-recommendation-import.spec.ts`
  - 种入资产数据到测试 SQLite
  - 覆盖：
    - 打开编辑器后推荐面板显示匹配资产
    - 点击 [插入] 将资产内容添加到编辑器
...
- ✅ Task 1: Asset 创建能力扩展 — `CreateAssetInput` type, `assetRepo.create()`, `assetService.create()`, `asset:create` IPC channel, preload exposure, `assetStore.createAsset()` action
- ✅ Task 2: 推荐引擎服务层 — `recommendation-types.ts`, `recommendation-service.ts` with FTS-based local matching, keyword extraction, content overlap filtering, `asset:recommend` IPC channel
- ✅ Task 3: 推荐状态管理 — `recommendationStore` with nonce-based stale response discard, section-scoped ignored/accepted sets, 2s debounce, `useAssetRecommendation` hook
- ✅ Task 4: 推荐面板与卡片组件 — `RecommendationPanel`, `RecommendationCard` (green #52C41A/#f6ffed), `RecommendationDetailDrawer` with asset:get detail loading
- ✅ Task 5: 一键入库交互 — `AssetImportDialog` with form pre-fill, `useAssetImport` hook, toolbar "一键入库" button with editor selection guard
- ✅ Task 6: 编辑器资产插入集成 — `InsertAssetFn` with 4-level fallback (selection → lastSelection → section end → doc end), PlateEditor `onInsertAssetReady` callback
- ✅ Task 7: 侧边栏集成 — RecommendationPanel integrated into AnnotationPanel shell as stacked section, ProjectWorkspace wires recommendation + insertAsset + detail drawer
- ✅ Task 8: 测试矩阵 — 195 test files, 1743 tests all passing. Full coverage of new code across repo/service/IPC/preload/store/component layers
- Extended `useCurrentSection` to configurable H1-H4 heading range (default H2-H4 for annotations, H1-H4 for recommendations)
- Extended `OutlineHeadingElement` to output H1 locator data attributes
- Extended `ChapterAwareHeading` locator computation to include H1
- Exported `computeMatchScores` from asset-service for shared use by recommendation-service
...
**Modified files:**
- src/shared/asset-types.ts — added CreateAssetInput
- src/shared/ipc-types.ts — added asset:create, asset:recommend channels + RecommendationContext/Result imports
- src/main/db/repositories/asset-repo.ts — added create() method
- src/main/services/asset-service.ts — added create() method, exported computeMatchScores
- src/main/ipc/asset-handlers.ts — added asset:create, asset:recommend handlers
- src/preload/index.ts — added assetCreate, assetRecommend methods
- src/renderer/src/stores/assetStore.ts — added createAsset() action
- src/renderer/src/stores/index.ts — exported useRecommendationStore
- src/renderer/src/modules/asset/index.ts — exported new components and hooks
- src/renderer/src/modules/annotation/hooks/useCurrentSection.ts — configurable minLevel/maxLevel
- src/renderer/src/modules/editor/components/OutlineHeadingElement.tsx — H1 locator attrs, ChapterAwareHeading H1 support
- src/renderer/src/modules/editor/components/EditorToolbar.tsx — 一键入库 button
- src/renderer/src/modules/editor/components/EditorView.tsx — currentSection prop, insertAsset bridge, import dialog
- src/renderer/src/modules/editor/components/PlateEditor.tsx — InsertAssetFn, onInsertAssetReady
- src/renderer/src/modules/project/components/AnnotationPanel.tsx — RecommendationPanel section
- src/renderer/src/modules/project/components/ProjectWorkspace.tsx — recommendation hook, insertAssetRef, detail drawer
...
- tests/unit/renderer/modules/editor/components/OutlineHeadingElement.test.tsx
- tests/unit/renderer/project/AnnotationPanel.test.tsx
- tests/unit/renderer/project/ProjectWorkspace.test.tsx

### Change Log

- 2026-04-12: Story 5.2 implemented — asset recommendation engine, one-click import, editor insertion, sidebar integration, full test matrix
