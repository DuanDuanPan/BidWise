...
│  │ 提交点语义  │  │ peek/ack │  │ pane检测  │  │ 决策包构建  │  │
│  │ 不变量前检  │  │ 游标消费  │  │ 健康检查  │  │ 规则注入   │  │
│  └───────────┘  └──────────┘  └──────────┘  └────────────┘  │
│  ┌───────────┐  ┌──────────┐  ┌──────────┐  ┌────────────┐  │
│  │ Watchdog   │  │ Pane Mgr │  │ State    │  │ generation │  │
│  │ 合规检测   │  │ tmux封装  │  │ Control  │  │ .lock      │  │
│  │ (简化)     │  │ (已有)    │  │ (内部用)  │  │ 唯一真相源  │  │
│  └───────────┘  └──────────┘  └──────────┘  └────────────┘  │
└──────────────────────────────────────────────────────────────┘
```
...
| Idle 无 MC_DONE | `PANE_IDLE_NO_SENTINEL` | P2 | **指挥官判断** |
| Worker 运行中 | 不写事件 | 无 | — |
...
applicable_rules:
  ...
  - "Check dispatch_state: packet_pasted → dispatch 修复; worker_running → 可能真完成"
  - "Check pane log for rate limit / content filter"
  - "Only TRANSITION dev_complete if positive evidence"
```
...
def process_command(expected_generation, raw_command)
  ...
  when :peek_events
    EventBus.peek(project_root, expected_generation, consumer: "commander", types: parsed.types)
  when :ack_events
    EventBus.ack(project_root, consumer: "commander", seq: parsed.seq)
  end
end
```
...
| 文件 | 行数 | 职责 |
|------|-----|------|
| `task-monitor.sh` | ~280 | IDLE 不自动推进 + 三层检测 |
| `monitor-control.sh` | ~100 | 生命周期 |
