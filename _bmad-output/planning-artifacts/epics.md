...
FR70: Epic 11 - 方案结构面板支持 Xmind 风格快捷键（Enter=同级 / Tab=子级 / Shift+Tab=升级 / Delete=级联删 / F2=编辑 / Esc=退出 / 方向键=导航），删除带 5s Toast Undo 撤销，章节内容同步级联软删除
FR71: Epic 11 - 基于招标文件解析结果（评分项、必响应项、需求矩阵）由 LLM 推荐方案结构，每个推荐节点附 sourceRequirement 字段标注招标依据
FR72: Epic 11 - 导入 Word 文档（.docx）提取目录骨架（Heading 样式 + 字号启发式双引擎），输出含置信度三档标注的结构供用户裁决导入
...
### Epic 11: 方案结构增强（快捷键 + AI 推荐 + Word 导入）
售前工程师可以用 Xmind 风格快捷键（Enter/Tab/Delete）高效编辑方案结构、误删后 5 秒内撤销；通过统一入口选择三条结构来源路径——通用模板（已有）、AI 招标驱动推荐、Word 文档目录提取，全部经 diff 合并视图让用户裁决接受/拒绝。
**FRs covered:** FR70（快捷键级联编辑）、FR71（结构 LLM 推荐）、FR72（Word 目录导入）
...
售前工程师可以用 Xmind 风格快捷键高效编辑方案结构、误删 5 秒内撤销；通过统一入口选择三条结构来源路径——通用模板（已有 Story 3.3）、AI 招标驱动推荐、Word 文档目录提取，全部经 diff 合并视图让用户裁决接受/拒绝。

> Story 11.1 为 Technical Enabler（章节稳定 ID 双层模型重构），交付双层 ID 基础后续故事方可启动；Story 11.2-11.5 为 Sprint A（快捷键体验完整交付）；Story 11.6-11.7 为 Sprint B（结构入口 + Word 导入）；Story 11.8 为 Sprint C（LLM 推荐）。

### Story 11.1: [Enabler] 章节稳定 ID 双层模型重构

As a 开发者,
I want 将方案章节 ID 从路径式（"2.1.3"）重构为稳定 UUID + derived path 双层模型,
So that 后续 Undo/多操作流水/LLM 推荐节点引用/Word 导入 ID 映射不会因结构变动而断裂。

**Acceptance Criteria:**

**Given** 现有方案章节存储使用路径式 ID
**When** 重构后
**Then** 数据库存储 `id`（UUID、稳定主键）+ `parentId` + `order`（同级排序号）+ `title`，**不存** path 字段

**Given** UI 渲染章节树
**When** 计算节点显示编号
**Then** 遍历树按 parent + order 计算 path（如 `2.1.3`），仅渲染层使用，不入库

**Given** 现有数据库有路径式 ID 历史数据
**When** 应用启动检测到旧 schema
**Then** 自动迁移：为每个旧节点生成 UUID、推导 parentId/order，更新所有引用方（章节内容文件名、sidecar JSON 引用、proposal.meta.json sectionWeights）

**Given** 节点 CRUD 操作
**When** 删除/移动/重排
**Then** UUID 永不变，仅 parent/order 更新，path 在下次渲染时自动重算

**Given** 跨 store 引用章节
**When** 任何引用方持久化（sectionWeights、annotations、scoring 关联）
**Then** 全部使用 UUID 作为外键，禁止存 path 字符串

### Story 11.2: 焦点节点视觉态 + 五状态机

As a 售前工程师,
I want 方案结构面板的当前焦点节点有清晰视觉反馈，并区分编辑、锁定、待删除等状态,
So that 我能瞬间识别"光标在哪"、"什么节点不能动"、"刚删了什么还没真消失"。

**Acceptance Criteria:**

**Given** 方案结构面板显示
**When** 节点获得焦点（↑↓导航或单击）
**Then** 节点显示焦点态：左侧 3px 主色条 + 浅底（`rgba(primary, 0.06)`）+ 键盘焦点 ring（`outline: 2px solid primary; outline-offset: 2px`）+ 右侧浮现 `[+ 子节点]` `[⋯ 更多]` 操作按钮

**Given** 节点处于焦点态
**When** 双击或按 F2
**Then** 进入 Editing 态：节点标题边框高亮 + 显示行内 Input + 文本光标聚焦

**Given** LLM 流式推荐生成中
**When** 该节点正在被填充
**Then** 进入 Locked 态：灰底 + 🔒 图标 + 节点禁用所有快捷键操作（Delete/Tab/Enter 全无效）

**Given** 节点被删除（Toast Undo 5s 窗口期）
**When** soft-delete 触发
**Then** 进入 Pending-Delete 态：红底 + 倒计时数字（5/4/3/2/1）+ 节点保留显示但不响应交互

**Given** 节点既无焦点也未编辑/锁定/待删
**When** 默认显示
**Then** 进入 Idle 态：常规样式，无装饰
...
**Given** 方案结构面板获焦
**When** 选中节点（非编辑态）按下快捷键
**Then** 执行对应操作：

| 键 | 行为 |
|---|---|
| `Enter` | 当前节点末尾插入同级新节点（自动进 Editing 态） |
| `Tab` | 当前节点缩进变为前一兄弟节点的子节点 |
| `Shift+Tab` | 当前节点反缩进变为父节点的兄弟 |
| `Delete` / `Backspace` | 删当前节点及其所有子节点（级联软删，触发 Toast Undo） |
| `F2` / 双击 | 进入 Editing 态 |
| `Esc` | 退出 Editing 态 |
| `↑/↓/←/→` | 节点导航（上下兄弟/父/首子） |

**Given** 节点处于 Editing 态
**When** 按下 Enter / Tab
**Then** Enter 在文本中插入换行（Shift+Enter 也是），Tab 仅在节点选中态生效（编辑态下不触发结构变更）

**Given** 快捷键作用域
**When** 焦点离开方案结构面板（如点击编辑器或其他面板）
**Then** 立即解绑全部结构相关快捷键，避免在 Plate 编辑器中按 Tab 触发结构变更

**Given** Tab 在第一个根节点上（无前序兄弟）
**When** 按下 Tab
**Then** 操作被拒绝（无前序节点可作为父），UI 不变化

**Given** 节点处于 Locked 态（LLM 流式生成中）
**When** 按下任意结构变更快捷键（Tab/Enter/Delete/Shift+Tab）
**Then** 操作被拒绝，显示 Toast 提示"AI 生成中，请稍候"

### Story 11.4: Soft-delete + Toast Undo（5s 撤销）

As a 售前工程师,
I want 删错节点 5 秒内能撤销，且撤销后章节正文内容也一并恢复,
So that 误操作不会让我丢失刚写的几千字。

**Acceptance Criteria:**

**Given** 用户按 Delete 删除节点（含子节点 + 章节正文）
**When** 删除触发
**Then** 在 SQLite 单一事务内为节点表 + 章节内容文件 + sidecar JSON 三者同步打 `deleted_at` 时间戳标记（soft-delete），**不**立即物理删除

**Given** 节点已 soft-delete
**When** UI 渲染
**Then** 节点进入 Pending-Delete 态（红底 + 倒计时），同时屏幕底部显示 Toast：
- 单节点无子：`"已删除「{title}」  [撤销]"`
- 单节点有子：`"已删除「{title}」及 {N} 个子节点（含正文 {字数} 字）  [撤销]"`

**Given** Toast 显示 5 秒内
**When** 用户点击 [撤销]
**Then** 清除所有 `deleted_at` 标记，节点 + 章节内容完整恢复，Toast 消失

**Given** Toast 显示 5 秒过期
**When** 时间到
**Then** 异步 GC 进程启动事务真删（节点表 + 章节内容文件 + sidecar JSON），不阻塞 UI

**Given** 5 秒撤销窗口期内 app 崩溃或被关闭
**When** 下次启动
**Then** 启动时立即扫描所有 `deleted_at IS NOT NULL` 的节点并执行 hard-delete（视为撤销窗口已过期），不保留半状态

**Given** 多次连续删除（前一次 Toast 未消失）
**When** 新删除触发
**Then** 旧 Toast 立即关闭并真删旧节点，新 Toast 替换显示，每次只保留一个 Undo 窗口
...
**Given** 当前节点处于第 6 层
**When** 用户按 Tab 创建第 7 层子节点
**Then** 操作允许执行，但弹出黄色警告 Toast："过深结构（{N} 层）会降低可读性，建议拆分为独立章节"

**Given** LLM 正在流式生成节点内容
**When** 节点处于生成中
**Then** 节点视觉进入 Locked 态（灰底 + 🔒 图标），所有结构变更快捷键失效

**Given** LLM 推荐结构生成
**When** Prompt 输出节点
**Then** Prompt 强制约束推荐结构深度 ≤6 层、节点数 ≤50（写入 `recommend-structure.prompt.ts` 的 system prompt）

**Given** Word 导入解析出深度 >6 的 Heading
**When** 预览导入
**Then** 深度 >6 的节点显示黄色警告标记，但允许导入（用户可在导入后调整）
...
**Given** 用户在三路径入口选择"导入 Word"
**When** 上传 .docx 文件
**Then** 主进程将文件 buffer 通过 IPC 传给 Python 子进程

**Given** Python FastAPI 收到 docx 内容
**When** 调用 `/api/extract-document-outline`
**Then** 用双引擎识别 Heading：
- 主引擎：`paragraph.style.name` 匹配 `Heading [1-9]`
- 备引擎（启发式）：font_size > 正文 + bold + 行长度 < 50 字符的段落
...
**Given** 提取结果
**When** 渲染预览界面
**Then** 显示提取的章节树，每节点旁标置信度 Tag（高=绿色、中=黄色、低=灰色），用户可勾选/取消接受

**Given** 用户确认导入
**When** 点击"导入到 diff 视图"
**Then** 进入 Story 11.6 的 diff 合并流程，与现有结构合并（仅取目录骨架，**不**导入 Word 正文内容）

**Given** Word 文件解析失败（损坏、密码保护、非 .docx）
**When** 错误发生
**Then** 显示明确错误信息 + 引导降级到其他路径
...
**Given** 项目已完成招标文件解析（Story 2.5/2.6/2.8 数据就绪）
**When** 用户在三路径入口选择"AI 推荐"
**Then** 主动触发（不自动后台跑），调用 `agent-orchestrator` 启动 LLM 推荐流程

**Given** Prompt 构造
**When** 输入招标解析结果给 LLM
**Then** 输入必先经 `desensitization-proxy`（NER + regex 抹甲方公司名/项目名/人名），符合 CLAUDE.md 强制脱敏要求

**Given** LLM 输出
**When** 流式 SSE 返回结构节点
**Then** 节点逐个流式渲染到 UI（用户可看见 LLM"思考"过程），每个节点为：
...
**Given** 推荐生成结束
**When** 进入 diff 视图
**Then** 复用 Story 11.6 的合并 UI，所有 AI 节点默认未勾选
...
**When** 错误发生
**Then** 显式失败 + 重试按钮 + 引导降级到其他两路径（不自动通用模板兜底，避免静默劣化）

**Given** Prompt 输出违反约束（深度 >6、节点数 >50）
**When** 解析返回
**Then** 主进程拒绝接受、记录日志、返回错误供 UI 重试
