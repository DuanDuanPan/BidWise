...
UX-DR9: 批注卡片组件——单条批注展示和操作容器，五色变体（蓝/绿/橙/红/紫），每色有专属操作按钮组（蓝:采纳/驳回/修改，绿:插入/忽略/查看，橙:处理/标记待决策，红:接受并修改/反驳/请求指导，紫:回复/标记已处理），状态流转待处理→已处理/已驳回/待决策，键盘 ↑↓ 导航+Enter 采纳+Esc 驳回
...
**Given** 应用启动
**When** Kysely 迁移执行
**Then** 自动创建 annotations 表（id, section_id, type, content, author, status, created_at）

**Given** 批注数据模型
**When** 创建批注
**Then** 包含 id、sectionId、type（ai-suggestion/score-warning/adversarial/human/cross-role/pending-decision）、content、author、status、createdAt 字段
...
**Given** 每种颜色的批注卡片
**When** 查看操作按钮
**Then** 蓝色：采纳/驳回/修改；绿色：插入/忽略/查看；橙色：处理/标记待决策；红色：接受并修改/反驳/请求指导；紫色：回复/标记已处理（UX-DR9）
...
**Given** 批注面板
**When** 查看计数器
**Then** 实时显示待处理批注数量，每处理一条计数器递减

**Given** 用户在批注线程中想向系统提问
**When** 点击"向系统提问"入口
**Then** 输入问题后系统基于产品能力基线/资产库 Streaming 回答，答案作为系统批注出现（UX-DR12）
